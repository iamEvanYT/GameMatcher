...
export async function createMatch(
  ...
  // create the match
  const insertedMatchId: ObjectId | null = await foundMatchesCollection
    .insertOne({
      teams,
      serverAccessToken,
      queueId,
...
export async function discoverMatches(queueId: string) {
  ...
  const services: Services = {
    getOldestParties: async (qid: string, limit = 2500) => {
      const docs = await queuesCollection
        .find({ queueId: qid })
        .sort({ timeAdded: 1 })
        .limit(limit)
        .toArray()
  ...
  await algo(queueData as any, services);
}
