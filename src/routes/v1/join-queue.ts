...
routes.post(
  "/",
  authMiddleware,
  parseJSONBody({ schema: JoinQueueSchema }),
  async (c: ContextWithParsedBody<typeof JoinQueueSchema>) => {
    const { partyId, userIds, queueId, rankedValue, serverAccessToken } = c.bodyData;
    ...
    const result = await queuesCollection
      .findOneAndUpdate(
        {
          _id: partyId
        },
